...
try {
    ...
    file_put_contents(__DIR__ . '/../data/err.txt', $e->getMessage());
...
function updateProfile($pdo, $storeId) {
    ...
    // Update session agar perubahan nama/logo langsung terlihat di sidebar
    $_SESSION['raz_store_name'] = $name;
    if ($logoFileName) $_SESSION['raz_store_logo'] = $logoFileName;
...
function updateReceipt($pdo, $storeId) {
    ...
    $invoiceFormat = trim(substr($data['invoice_format'] ?? 'INV-{Ymd}-{SEQ5}', 0, 50));
    if (empty($invoiceFormat)) $invoiceFormat = 'INV-{Ymd}-{SEQ5}';

    $stmt = $pdo->prepare("UPDATE stores SET receipt_template=?, receipt_header=?, receipt_footer=?, receipt_show_logo=?, invoice_format=? WHERE id=?");
    $stmt->execute([$template, $header, $footer, $showLogo, $invoiceFormat, $storeId]);
