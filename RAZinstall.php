    ...
    if (empty($adminUser) || empty($adminPass)) {
        ...
        try {
            ...
            $pdo->exec("CREATE TABLE IF NOT EXISTS stores (
                ...
                invoice_format VARCHAR(50) DEFAULT 'INV-{Ymd}-{SEQ5}',
            ...
            $pdo->exec("CREATE TABLE IF NOT EXISTS hpp_calculations (
                id {$autoInc},
                store_id INT NOT NULL,
                product_name VARCHAR(150) NOT NULL,
                portions INT DEFAULT 1,
                overhead_pct {$decimalType} DEFAULT 15,
                margin_pct {$decimalType} DEFAULT 30,
                current_sell_price {$decimalType} DEFAULT 0,
                notes {$textType},
                is_active TINYINT(1) DEFAULT 1,
                created_at {$timestamp},
                updated_at {$timestamp}
            ...
            $pdo->exec("CREATE TABLE IF NOT EXISTS hpp_ingredients (
                id {$autoInc},
                hpp_id INT NOT NULL,
                name VARCHAR(100) NOT NULL,
                purchase_qty {$decimalType} DEFAULT 1,
                purchase_unit VARCHAR(30) DEFAULT 'pcs',
                purchase_price {$decimalType} DEFAULT 0,
                portions_yield INT DEFAULT 1,
                sort_order INT DEFAULT 0
            )");

            $pdo->exec("CREATE TABLE IF NOT EXISTS hpp_packagings (
                id {$autoInc},
                hpp_id INT NOT NULL,
                name VARCHAR(100) NOT NULL,
                purchase_qty {$decimalType} DEFAULT 1,
                purchase_unit VARCHAR(30) DEFAULT 'pack',
                purchase_price {$decimalType} DEFAULT 0,
                capacity_pcs INT DEFAULT 1,
                usage_per_portion INT DEFAULT 1,
                sort_order INT DEFAULT 0
            )");

            $pdo->exec("CREATE TABLE IF NOT EXISTS hpp_extra_costs (
                id {$autoInc},
                hpp_id INT NOT NULL,
                name VARCHAR(100) NOT NULL,
            ...
            <div class="install-body">
                <?php if ($installSuccess): ?>
                    ...
                    <div class="install-success">
                        ...
                        <a href="RAZlogin.php" class="raz-btn raz-btn-success raz-btn-block">
